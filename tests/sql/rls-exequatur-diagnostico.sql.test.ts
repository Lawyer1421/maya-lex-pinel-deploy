/**
 * PGlite — 17 escenarios de seguridad Slice 3B (S09–S17).
 * Cambio real de roles (SET ROLE). Identidad = auth.uid().
 * No aplica SQL en prod.
 */
...
beforeAll(async () => {
  ...
  await db.exec(`
    ...
    -- Reproduce BYPASSRLS de Supabase para probar que, aun así,
    -- sin GRANT ni política, service_role no entra por SQL de app.
...
describe('17 escenarios de seguridad — PGlite con SET ROLE (S09–S17)', () => {
  let intentoA: string;

  it('S09 ambas tablas tienen RLS + FORCE', async () => {
  ...
  it('S10 A puede insertar su propio intento (user_id = auth.uid())', async () => {
  ...
  it('S11 A no puede insertar con user_id de B (WITH CHECK)', async () => {
    await asAuthenticated(USER_A);
    await expect(db.query(insertIntentoSql(`'${USER_B}'::uuid`))).rejects.toThrow();
    await resetToSuperuser();
  });

  it('S12 A lee su fila y no la de B', async () => {
    await resetToSuperuser();
  ...
  it('S13 A puede upsert su progreso y no el de B', async () => {
    await asAuthenticated(USER_A);
    await db.exec(`
  ...
  it('S14 authenticated no tiene UPDATE/DELETE sobre intentos', async () => {
    await asAuthenticated(USER_A);
    await expect(
  ...
  it('S15 aunque se otorgue UPDATE, RLS bloquea (no hay política UPDATE)', async () => {
    await resetToSuperuser();
  ...
  it('S16 anon no puede leer ni insertar', async () => {
    await asRole('anon');
    await expect(
  ...
  it('S17 cero políticas/GRANT de service_role; SET ROLE service_role no lee', async () => {
    const policies = await db.query<{ polname: string; roles: string }>(
      `select p.polname, coalesce(array_to_string(array(
         select r.rolname from unnest(p.polroles) u(oid)
         join pg_roles r on r.oid = u.oid
       ), ','), '') as roles
         from pg_policy p
         join pg_class c on c.oid = p.polrelid
        where c.relname in ('exequatur_diagnostico_intentos', 'exequatur_diagnostico_progreso')`,
    );
    expect(policies.rows.length).toBeGreaterThan(0);
    for (const row of policies.rows) {
      expect(row.roles).not.toMatch(/service_role/);
      expect(row.polname).not.toMatch(/service/i);
    }
    ...
    const grants = await db.query<{ grantee: string }>(
      `select grantee
        ...
        where table_schema = 'public'
          and table_name in ('exequatur_diagnostico_intentos', 'exequatur_diagnostico_progreso')`,
    );
    expect(grants.rows.every((row) => row.grantee !== 'service_role')).toBe(true);
