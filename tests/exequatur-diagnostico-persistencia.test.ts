...
describe('esUuid / parsearIntentoQuery', () => {
  it('acepta UUID v4 y rechaza basura', () => {
...
describe('resolverSesionExequatur — identidad de sesión', () => {
  it('sin usuario → null', async () => {
  ...
  it('usuario sin acceso Exequátur → null', async () => {
  ...
  it('userId sale de auth.getUser().id, no del email', async () => {
    mockCreateClient.mockResolvedValue(
      fakeClient({ user: { id: USER_A, email: 'a@test.com' } }) as never,
    );
...
describe('guardarIntento', () => {
  it('inserta user_id de la sesión y re-evalúa respuestas (ignora score de cliente)', async () => {
    ...
    expect(inserted[0]?.respuestas).toEqual(respuestas);
  ...
  it('error de insert → fail-closed null (sin fallback)', async () => {
    mockCreateClient.mockResolvedValue(
      fakeClient({ insertError: 'relation does not exist' }) as never,
    );
    await expect(guardarIntento(SESION_A, {})).resolves.toBeNull();
  });
});

describe('cargarIntentoPropio — IDOR + re-evaluación', () => {
  it('UUID inválido no consulta la tabla', async () => {
  ...
  it('intento ajeno filtrado por user_id → null', async () => {
    mockCreateClient.mockResolvedValue(
      fakeClient({
        intentos: [
          {
  ...
  it('re-evalúa respuestas y no usa aciertos persistidos', async () => {
    const respuestas = { 'item-notariado-institucion': 'c' };
    mockCreateClient.mockResolvedValue(
      fakeClient({
        intentos: [
          {
  ...
  it('claves extra o no-string en respuestas se ignoran', async () => {
    mockCreateClient.mockResolvedValue(
      fakeClient({
        intentos: [
          {
...
describe('cargarUltimoIntentoPropio', () => {
  it('solo considera filas de la sesión', async () => {
    mockCreateClient.mockResolvedValue(
      fakeClient({
        intentos: [
          {
